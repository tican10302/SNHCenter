...
public class WardRepository(IUnitOfWork unitOfWork, IMapper mapper, DataContext context) : IWardRepository
{
    ...
    public async Task<WardModel> GetById(string request)
    {
        var data = await context.Wards.FindAsync(request);
        if (data == null)
        {
